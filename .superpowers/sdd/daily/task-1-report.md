...
- Added `checkout_attempt_id` and `checkout_started_at` to table sessions.
...
- Recovery also returns the failed attempt's reserved promo use, clears failed-checkout payment methods on still-pending orders, and resets checkout metadata.
- Updated the Edge Function to call the matching release RPC for missing Stripe/VNPay configuration, Stripe rejection, Stripe fetch/parse/timeout exceptions, and VNPay signing exceptions. Successful gateway setup is not released.
...
Same focused command after implementation: exit 0; 2 files passed, 31 tests passed. Covered:
...
Database pgTAP coverage was added for the database-only contracts: mismatched attempt refusal, matching unfinished release, completed-attempt refusal, returned/persisted gateway attempt id, manager update denial, admin update success, and all three constraints.

## Full verification

- `npx eslint lib/supabase/settings-data.ts lib/supabase/settings-data.test.ts components/admin/settings-view.tsx supabase/functions/checkout-table-session/index.test.ts` — exit 0.
- `git diff --check` — exit 0.
- `npx tsc --noEmit` — exit 0.
- `npm test` — exit 1 with 257 passed and 1 failed. The sole failure is outside Task 1: the other session's untracked `components/ui/dialog.tsx` calls `Dialog.close`, `Dialog.cancel`, `Dialog.working`, and `Dialog.confirm`, which are not yet present in either catalog. All 32 other test files passed, including every Task 1 test.
...
- Re-run the full Vitest suite after Task 6 finishes its Dialog catalog entries; Task 1's full-suite run was otherwise 257/258.
