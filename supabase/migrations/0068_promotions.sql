-- 0068_promotions.sql
-- Real promotions/coupon system, replacing the hardcoded WELCOME10 that
-- was independently duplicated in hooks/useCart.tsx (client) and
-- place_order (server, migration 0067). Design:
-- docs/superpowers/specs/2026-08-03-promotions-design.md
--
-- No public SELECT policy: a promo code is typed, not browsed (unlike
-- the public rewards catalog) -- validate_promo_code (security
-- definer) is the only way to check one exact code, same exposure as
-- brute-forcing any coupon system, not enumeration.
...
create policy "promotions_manager_admin_all" on public.promotions
  for all
...
-- Guest-safe: checks exactly one code, no role gate (same shape as
-- get_order_for_tracking/cancel_pending_order).
create or replace function public.validate_promo_code(p_code text, p_subtotal integer)
returns json
language plpgsql
security definer
...
declare
  ...
  v_amount integer;
begin
  select * into p from public.promotions where code = v_code;
  ...
  if p.max_redemptions is not null and p.times_used >= p.max_redemptions then
    return json_build_object('valid', false, 'reason', 'limit_reached');
  end if;
  if p.min_subtotal_vnd is not null and p_subtotal < p.min_subtotal_vnd then
    return json_build_object('valid', false, 'reason', 'below_minimum');
  end if;

  v_amount := case p.discount_type
    when 'percent' then round(p_subtotal * p.discount_value / 100.0)
    else p.discount_value
  end;
  v_amount := least(v_amount, greatest(p_subtotal, 0));
...
-- Full redefinition of place_order (unchanged body from migration 0067
-- except: the WELCOME10 branch is replaced with a real, row-locked
-- promotions lookup; orders gains promo_code; times_used increments on
-- success).
